...
namespace BaseCore.AuthService.Controllers
{
    ...
    // Role hợp lệ — dùng chung cho Create và Update để tránh typo.
    file static class Roles
    {
        internal static readonly HashSet<string> Valid =
            new(StringComparer.Ordinal) { "Admin", "WarehouseStaff", "Marketing", "Customer" };
    }
    ...
    public class UserController : ControllerBase
    {
        ...
        public async Task<IActionResult> Create([FromBody] CreateUserRequest request)
        {
            ...
            if (!Roles.Valid.Contains(request.Role))
                return BadRequest(new { message = $"Role không hợp lệ. Chỉ chấp nhận: {string.Join(", ", Roles.Valid)}." });
