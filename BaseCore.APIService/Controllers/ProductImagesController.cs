...
namespace BaseCore.APIService.Controllers
{
    ...
    public class ProductImagesController : ControllerBase
    {
        ...
        {
            if (!string.IsNullOrWhiteSpace(product.Slug)) return;
            var baseSlug = SlugHelper.Generate(product.Name ?? string.Empty);
            if (string.IsNullOrWhiteSpace(baseSlug)) baseSlug = $"san-pham-{product.Id}";
            var slug = baseSlug;
            var i = 2;
            while (await _context.Products.AnyAsync(p => p.Slug == slug && p.Id != product.Id))
                slug = $"{baseSlug}-{i++}";
            product.Slug = slug;
            await _context.SaveChangesAsync();
        }
