...
const OrderConfirmationPage = () => {
  ...
  // Đơn của khách vãng lai được truyền qua navigation state (guest không có
  // quyền gọi GET /orders/{id}); thành viên thì tải lại từ server.
  ...
  // ════════════════════════════════════════════════════════════
  useEffect(() => {
    // Đã có đơn từ state (guest vừa đặt) → không cần gọi API.
    if (stateOrder) return undefined;
      ...
      <div className="mx-auto max-w-3xl px-4 py-16 sm:px-6 lg:px-8">
        ...
        <div className="rounded-3xl bg-gradient-to-br from-slate-50 via-white to-slate-50 p-10 text-center shadow-sm ring-1 ring-slate-100">
          ...
          <p className="mt-3 text-slate-600">
            Mã đơn hàng <span className="font-semibold text-slate-900">{order?.orderCode || `#${orderId}`}</span>
          </p>
          {order?.orderCode && (
            <p className="mt-1 text-xs text-slate-400">
              Lưu mã này để tra cứu đơn hàng bất cứ lúc nào.
            </p>
          )}
        </div>
        ...
        <div className="mt-10 flex flex-col justify-center gap-3 sm:flex-row">
          {isAuthenticated ? (
            ...
            <Link to="/track" className="rounded-xl bg-slate-900 px-8 py-3 text-center text-sm font-semibold uppercase tracking-wider text-white transition hover:bg-slate-700">
              Tra cứu đơn hàng
            </Link>
          )}
