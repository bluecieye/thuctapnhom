...
import { CreditCard, MapPin, Tag, Check, Truck, Trash2 } from 'lucide-react';
...
const PROVINCE_LIST_URL = `${PROVINCES_API}/`;
const provinceWardsUrl = (code) => `${PROVINCES_API}/p/${code}?depth=2`;
...
const CheckoutPage = () => {
  ...
  const { cartItems, totalPrice, totalItems, refresh, clearCart } = useCart();

  const { isAuthenticated, user } = useAuth();

  // Khách vãng lai (chưa đăng nhập) → checkout bằng địa chỉ nhập trực tiếp.
  ...
  const [codeToDbId, setCodeToDbId] = useState({});

  const [newAddress, setNewAddress] = useState({
    fullName: '', phone: '', street: '', ward: '', provinceCode: '', isDefault: true,
  });

  // Email nhận xác nhận đơn (bắt buộc với khách vãng lai).
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      // Tải danh sách tỉnh từ external API (post-merger 2025) + DB map để resolve FK.
      const [extRes, dbRes] = await Promise.all([
        fetch(PROVINCE_LIST_URL).then((r) => r.json()).catch(() => []),
        provincesApi.getAll().then((r) => r.data || []).catch(() => []),
      ]);
      setProvinces(extRes);
      const map = {};
      dbRes.forEach((p) => { if (p.code) map[p.code] = p.id; });
      setCodeToDbId(map);

      // Khách vãng lai: KHÔNG gọi sổ địa chỉ (cần đăng nhập). Chỉ tải dữ liệu công khai.
      if (isGuest) {
        const c = await shippingCarriersApi.getAll(true);
        setCarriers(c.data || []);
      ...
      // Thành viên: tải thêm sổ địa chỉ.
      ...
      // Prefill email từ tài khoản (có thể chỉnh lại).
      if (user?.email) setEmail(user.email);
    })();

  }, [isGuest, user]);

  // Tải danh sách xã/phường khi tỉnh thay đổi.
  useEffect(() => {
    if (!newAddress.provinceCode) { setWards([]); return; }
    fetch(provinceWardsUrl(newAddress.provinceCode))
      .then((r) => r.json())
  ...
  // Map ngược: dbProvinceId → apiCode (để resolve code cho user đã đăng nhập từ provinceId của address).
  const dbIdToCode = useMemo(
    () => Object.fromEntries(Object.entries(codeToDbId).map(([code, id]) => [String(id), code])),
    [codeToDbId]
  );

  // Mã tỉnh dùng để tính cước: thành viên lấy từ provinceId của địa chỉ đã chọn (qua map),
  // khách vãng lai lấy từ form nhập trực tiếp.
  const shippingProvinceCode = isGuest
    ? (newAddress.provinceCode || null)
    : (dbIdToCode[String(selectedAddress?.provinceId)] || null);
  ...
  useEffect(() => {
    if (!shippingProvinceCode) { setQuotes([]); return; }
  ...
  }, [shippingProvinceCode, subtotalAfterDiscount]);
  ...
  const deleteAddress = async (id) => {
    try {
      ...
      if (addressId === id) {
        const def = refreshed.find((x) => x.isDefault) || refreshed[0];
        if (def) setAddressId(def.id);
        else { setAddressId(null); setShowNewAddress(true); }
      }
    } catch {
      setError('Không xoá được địa chỉ.');
    }
  };

  const saveNewAddress = async () => {
    setError('');
    const resolvedProvinceId = codeToDbId[newAddress.provinceCode];
    if (!resolvedProvinceId) {
      setError('Tỉnh/thành không hợp lệ. Vui lòng chọn lại.');
      return;
    }
    try {
      const { provinceCode: _unused, ...rest } = newAddress;
      const payload = { ...rest, provinceId: resolvedProvinceId };
  ...
  const handleSubmit = async (e) => {
    ...
    if (isGuest) {
      // Khách vãng lai: kiểm tra email + địa chỉ nhập trực tiếp.
      if (!emailOk) {
        setError('Vui lòng nhập email hợp lệ để nhận xác nhận đơn hàng.');
        return;
      }
      if (!newAddress.fullName || !newAddress.phone || !newAddress.street || !newAddress.provinceCode) {
    ...
    try {
      ...
      const order = await orderService.createOrder({
        // Thành viên dùng addressId; khách vãng lai gửi địa chỉ trực tiếp + items.
        ...
        ...(isGuest && {
          ...
          guestStreet: newAddress.street,
          guestWard: newAddress.ward,
          guestProvinceId: codeToDbId[newAddress.provinceCode],
      ...
      // Thành viên: đồng bộ giỏ server. Khách: xoá giỏ localStorage.
      if (isGuest) await clearCart();
      else await refresh();

      navigate(`/order-confirmation/${order.id}`, {
        replace: true,
        // Truyền cả order để guest xem xác nhận mà không cần quyền gọi GET /orders/{id}.
        ...
        <div className="lg:grid lg:grid-cols-12 lg:gap-8">
          ...
          <form onSubmit={handleSubmit} className="space-y-6 lg:col-span-7">
            ...
            {/* ════════════════════════════════════════════════════════════
                RENDER: FORM ĐỊA CHỈ
                ════════════════════════════════════════════════════════════ */}
            <Section icon={MapPin} title="Địa chỉ giao hàng" subtitle={isGuest ? 'Nhập thông tin người nhận' : 'Chọn từ sổ địa chỉ hoặc thêm mới'}>
              {/* Email nhận xác nhận đơn — bắt buộc với khách vãng lai. */}
              <div className="mb-4">
                <label className="mb-1 block text-xs font-medium uppercase tracking-wider text-slate-500">
                  Email nhận xác nhận {isGuest && <span className="text-rose-500">*</span>}
                </label>
                <input type="email" className={inputClass} placeholder="email@example.com"
                  value={email} onChange={(e) => setEmail(e.target.value)} />
                <p className="mt-1 text-[11px] text-slate-400">Mã đơn & chi tiết đơn hàng sẽ được gửi tới email này.</p>
              </div>
                      ...
                      <label key={a.id}
                        ...
                        <div className="flex-1">
                          ...
                          <p className="text-sm text-slate-600">
                            {a.street}, {a.ward}, {provinces.find((p) => String(p.code) === dbIdToCode[String(a.provinceId)])?.name || ''}
                          </p>
                        </div>
                        <button
                          type="button"
                          onClick={(e) => { e.preventDefault(); deleteAddress(a.id); }}
                          className="ml-2 shrink-0 text-slate-400 hover:text-rose-600 transition-colors"
                          title="Xoá địa chỉ"
                        >
                          <Trash2 size={16} />
                        </button>
              ...
              {(isGuest || showNewAddress) && (
                <div className="mt-4 space-y-3 rounded-xl bg-slate-50 p-4">
                  ...
                  {}
                  <div className="grid grid-cols-1 gap-3 sm:grid-cols-2">
                    <select className={inputClass} value={newAddress.provinceCode}
                      onChange={(e) => setNewAddress({ ...newAddress, provinceCode: e.target.value, ward: '' })}>
                      <option value="">Tỉnh/Thành phố</option>
                      {provinces.map((p) => <option key={p.code} value={String(p.code)}>{p.name}</option>)}
                    </select>
                    <select className={inputClass} value={newAddress.ward}
                      onChange={(e) => setNewAddress({ ...newAddress, ward: e.target.value })}
                      disabled={!newAddress.provinceCode || wards.length === 0}>
                      <option value="">{newAddress.provinceCode && wards.length === 0 ? 'Đang tải...' : 'Phường/Xã'}</option>
                      {wards.map((w) => <option key={w.code} value={w.name}>{w.name}</option>)}
                    </select>
                  </div>
                  {/* Lưu vào sổ địa chỉ chỉ áp dụng cho thành viên. */}
                  {!isGuest && (
            ...
            {}
            <button type="submit" disabled={loading || (!isGuest && !addressId) || !selectedQuote?.available}
