...
const Footer = () => {
  ...
  const handleSubscribe = async (e) => {
    ...
    try {
      ...
      show(err.response?.data?.message || 'Có lỗi xảy ra, vui lòng thử lại.', 'error');
    } finally {
      setLoading(false);
    }
  };
            ...
            <form onSubmit={handleSubscribe} className="mt-4">
              <div className="flex">
                <input
                  ...
                  disabled={loading}
                  className="flex-1 border border-gray-200 px-4 py-2 text-sm outline-none focus:border-black disabled:bg-gray-50"
                />
                <button
                  type="submit"
                  disabled={loading}
                  className="bg-black px-4 py-2 text-sm text-white hover:bg-gray-800 disabled:bg-gray-400"
                >
                  {loading ? '...' : 'Đăng Ký'}
                </button>
              </div>
