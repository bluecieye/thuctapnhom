...
const emptyForm = { name: '', description: '', basePrice: 0, categoryId: '' };
...
export const ProductsSection = () => {
  ...
  const openEdit = (p) => {
    setEditing(p);
    setFormData({ name: p.name, description: p.description || '', basePrice: p.basePrice || 0, categoryId: p.categoryId });
    setActiveTab('info'); 
    setError('');
    setShowModal(true);
  };
  ...
  const submit = async (e) => {
    ...
    try {
      ...
      const body = { ...formData, basePrice: parseFloat(formData.basePrice) || 0, categoryId: parseInt(formData.categoryId) };
        ...
        <>
          ...
          <AdminTable
            columns={[
              ...
              { key: 'price',    label: 'Giá', render: (p) => <span className="font-semibold">{fmt(p.basePrice)} đ</span> },
